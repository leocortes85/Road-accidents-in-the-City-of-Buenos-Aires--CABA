...
def data_summ(df):
    ...
    print("\nTotal full duplicate rows: ", df.duplicated().sum())
...
def count_value_per_column(dataframe, value_to_count):
    """
    Count the occurrences of a specific value in each column of the DataFrame.
    ...
    Prints:
    - The count of the specified value in each column.

    Example:
    - count_value_per_column(df, 'SD')  # Replace 'df' with your DataFrame name.
    """
    for column_name in dataframe.columns:
        count_value = dataframe[column_name].value_counts().get(value_to_count, 0)
        print(f"Count of '{value_to_count}' in column {column_name}: {count_value}")
